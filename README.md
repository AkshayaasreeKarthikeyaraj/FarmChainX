...
### 4. QR Scanning Setup For Other Devices
QR codes are generated by the backend and must contain a reachable frontend URL.

- For same Wi-Fi testing (LAN), set `FRONTEND_URL` before starting backend:
	- PowerShell: `$env:FRONTEND_URL="http://<your-laptop-ip>:4200"`
- Start frontend on LAN so phones can open it:
	- `ng serve --host 0.0.0.0 --port 4200 --ssl false`
- Generate new QR codes after setting `FRONTEND_URL`.

Notes:
- Local IP URLs (`192.168.x.x`) work only on the same Wi-Fi.
- For access from anywhere, use your deployed frontend domain as `FRONTEND_URL` and regenerate QR codes.
